...
$executeAuto=shell_exec('AutomationPage.exe http://www.bhinneka.com/categories.aspx '. $ShowProgress . ' '. 'AUTOPAGE');
$html_Bhineka = new simple_html_dom();
$html_Bhineka->load($executeAuto);

try 
{
	foreach($html_Bhineka->find('li.header') as $element) 
	{
		
	    $LinkCategory[0]="http://www.bhinneka.com" . $element->find('a',0)->href;
	        ...
	        for ($i=1; $i<=$MaxPage; $i++)
	        {
				...
				foreach($Content_Base_Bhineka->find('li.prod-itm') as $Contentelement) 
				{
					...
					if($resultKeywordPass==1)
					{
						...
						$ResultKeyword=3;
						$sql=mysqli_query($conn, "INSERT INTO tblcrawl(ProductLink,ImageLink,ProductName,ProductPrice,Source) VALUES('{$linkBox[0]}','{$Image[0]}','{$ProductName[0]}','{$RemoveCharacterPrice}','BHINEKA','{$Seller[0]}')");
