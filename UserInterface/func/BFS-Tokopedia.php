...
$resultKeywordPass=0;
$isFirstTime=0;
$executeAuto=shell_exec('AutomationPage.exe https://www.tokopedia.com/p '. $ShowProgress . ' '. 'AUTOPAGE');
...
try 
{
	foreach($html_base_Tokopedia->find('ul.rel-category-wrapper_allcat') as $element) 
	{
		foreach($element->find('a.font-bold') as $linkPerCategory)
		{
			$LinkCategory[0]=$linkPerCategory->href;
			$LinkValue[0]=$linkPerCategory->innertext;
